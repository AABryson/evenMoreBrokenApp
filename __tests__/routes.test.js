...
//generate token for the user based on username and admin
...
//######this one doesn't have an error test as well?????????????????????????????????????????????????
//############!!!!!!!!!!!!!!!!!!looks fine
...
//#######!!!!!!!!!!!!!!!!!this looks fine
  ...
  //TEST BUG #4 : auth/login returns the passowrd user logged in with.  This is security issue.
  test('should not return user password after logging in', async function () {
...
describe("GET /users", function() {
  ...
  //BUG 2: doesn't deny access if no token since doesn't have jwt.verify method
  //TEST BUG #2 : Use user 'u1' to show that user can get access even if token is not verified.  Use wrong token.
  test('will not get error when use invalid token', async function () {
    const response = await request(app).get("/users?token=faketokenString");
    // const response = await (await request(app).get("/users").set('Authorization', 'Bearer invalid_token'));
    // expect(response.statusCode).toBe(200);
  })
})
...
//TEST BUG #5: the .get('/users') route
//if username is passed in requ.body
