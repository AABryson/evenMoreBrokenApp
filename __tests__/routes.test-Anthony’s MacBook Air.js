...
//#execut db.query inside loop;
    await db.query(
//does user not need to be in an array? sampleUsers is an array;
      `INSERT INTO users VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      user
    );
//generate token for the user basede on username and admin
...
//#does this return error b/c don't have .send('authorization', 'Bearer )
  ...
  test("should list all users", async function() {
    ...
    expect(response.statusCode).toBe(200);
//#??????????????????????isn't users an object and thus can't have length
...
//#does this fail b/c don't have .send?
    expect(response.statusCode).toBe(401);
  });

//#this tests if right user.  I doesn't seem to test is not admin
...
//#why include .send when its wrong username?
